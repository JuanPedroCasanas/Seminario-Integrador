...
// formato de dia y eso (para la vista)
...
// cancelar turno
  const cancelAppointment = async (idAppointment: number) => {
    ...
    try {
      ...
      const data = await res.json();
      setToast({ message: data?.message ?? 'Turno cancelado', type: 'success' });

      // Mantener visible en "Próximos" con estado Cancelado
      setAppointments(prev =>
        prev.map(a => (a.id === idAppointment ? ({ ...a, status: 'canceled' } as any) : a)),
      );
      ...
      >
        <div className="flex items-start justify-between gap-3">
          <div className="min-w-0">
            ...
            <p className="text-sm text-gray-900">
              <span className="font-medium">{fullName(first.professional)}</span>
  ...
  return (
    <div>
      <div className="p-4 sm:p-5">
        <h3 className="text-base sm:text-lg font-semibold text-gray-900 mb-3">Mis turnos</h3>
        ...
        {!loading && upcoming.length > 0 && (() => {
          ...
          return (
            <section className="mb-4">
              <h4 className="text-sm font-semibold text-gray-800 mb-2">Próximos</h4>
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  {seriesMap.size > 0 ? (
                    ...
                    <p className="text-sm text-gray-500 italic">No tenés turnos sostenidos</p>
                  )}
                </div>
                ...
                <div>
                  ...
                  {singles.length > 0 ? (
                    ...
                    <p className="text-sm text-gray-500 italic">No tenés turnos únicos</p>
                  )}
                </div>
              </div>
            </section>
        ...
        {!loading && past.length > 0 && (() => {
          ...
          return (
            <section>
              <h4 className="text-sm font-semibold text-gray-800 mb-2">Anteriores</h4>
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  {seriesMap.size > 0 ? (
                    ...
                    <p className="text-sm text-gray-500 italic">No tenés turnos sostenidos anteriores</p>
                  )}
                </div>
                ...
                <div>
                  ...
                  {singles.length > 0 ? (
                    <ul className="space-y-3">
                      {past.map(renderPastItem)}
                    </ul>
                  ) : (
                    <p className="text-sm text-gray-500 italic">No tenés turnos únicos anteriores</p>
                  )}
                </div>
              </div>
            </section>
