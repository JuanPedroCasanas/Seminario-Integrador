...
export default function PatientAppointmentsCard() {
  ...
  useEffect(() => {
    const load = async () => {
      ...
      setLoading(true);
      try {
        const res = await authFetch(`${API_BASE}/appointment/getAppointmentsByPatient/${myPatientId}`);
        if (!res.ok) {
        ...
        console.error(err);
        setToast({ message: 'Error al cargar tus turnos', type: 'error' });
      } finally {
        setLoading(false);
      }
    };
    ...
    try {
      ...
      if (appointmentToCancel.series?.id) {
        ...
        const res = await authFetch(`${API_BASE}/appointment/updateStatus`, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ idAppointment: appointmentToCancel.id, status: 'canceled' as const }),
        });
        ...
        const data = await res.json();
        setToast({ message: data?.message ?? 'Turno cancelado exitosamente', type: 'success' });
      ...
      >
        <div className="space-y-1 mb-3">
          ...
          <p className="text-sm text-gray-800">
            <span className="font-medium">Fecha:</span> {a.startTime ? formatDateES(a.startTime) : '—'}
          </p>
          <p className="text-sm text-gray-800">
            <span className="font-medium">Horario:</span> {a.startTime ? formatTimeES(a.startTime) : '—'}
          </p>
          <p className="text-sm text-gray-800">
            <span className="font-medium">Profesional:</span> {fullName(a.professional)}
          </p>
          <p className="text-sm text-gray-800">
            <span className="font-medium">Consultorio:</span> {a.consultingRoom?.description ?? '—'}
          </p>
          <p className="text-sm text-gray-800">
            <span className="font-medium">Tipo:</span> {isSeries ? 'Sostenido' : 'Único'}
          </p>
        </div>
        <PrimaryButton
          size="sm"
  ...
  const renderSeriesAppointment = (seriesId: number, seriesAppointments: PopulatedAppointment[]) => {
    ...
    return (
      ...
      >
        <div className="space-y-1 mb-3">
          ...
          <p className="text-sm text-gray-800">
            <span className="font-medium">Día:</span> {first.startTime ? getDayOfWeekES(first.startTime) : '—'}
          </p>
          <p className="text-sm text-gray-800">
            <span className="font-medium">Horario:</span> {first.startTime ? formatTimeES(first.startTime) : '—'}
          </p>
          <p className="text-sm text-gray-800">
            <span className="font-medium">Profesional:</span> {fullName(first.professional)}
          </p>
          <p className="text-sm text-gray-800">
            <span className="font-medium">Consultorio:</span> {first.consultingRoom?.description ?? '—'}
          </p>
          <p className="text-sm text-gray-800">
  ...
  return (
    <div>
      ...
      {!loading && upcomingScheduled.length === 0 && (
        <EmptyState
          title="No tenés turnos reservados"
          description="Cuando reserves un turno, va a aparecer acá."
          icon={
            <svg className="w-12 h-12 text-cyan-600" viewBox="0 0 24 24" aria-hidden="true">
              <path fill="currentColor" d="M12 12a5 5 0 1 0-5-5 5 5 0 0 0 5 5m-8.5 8a8.5 8.5 0 0 1 17 0z" />
            </svg>
          }
        />
      )}
      ...
      {cancelModalOpen && appointmentToCancel && (
        <Modal title="¿Está seguro que desea cancelar el turno?" onClose={closeCancelModal}>
          <div className="space-y-1 mb-4">
            <p className="text-sm text-gray-600">
              <span className="font-semibold">Turno {appointmentToCancel.series?.id || appointmentToCancel.id}</span>
            </p>
            <p className="text-sm text-gray-800">
              <span className="font-medium">{appointmentToCancel.series?.id ? 'Día:' : 'Fecha:'}</span>{' '}
            ...
            <p className="text-sm text-gray-800">
              ...
              {appointmentToCancel.startTime ? formatTimeES(appointmentToCancel.startTime) : '—'}
            </p>
            <p className="text-sm text-gray-800">
              <span className="font-medium">Profesional:</span> {fullName(appointmentToCancel.professional)}
            </p>
            <p className="text-sm text-gray-800">
            ...
            <p className="text-sm text-gray-800">
              <span className="font-medium">Tipo:</span> {appointmentToCancel.series?.id ? 'Sostenido' : 'Único'}
            </p>
          </div>
